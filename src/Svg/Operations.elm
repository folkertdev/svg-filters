module Svg.Operations exposing (..)
...
Notable omissions from this module are the diffuse and specular lights

# Combining Filters
@docs sequential, concurrent

# Color
@docs flood, tile, colorMatrix
...
# Composite
@docs arithmetic, over, in_, atop, xor, out

# Noise
@docs displacement, gaussianBlur
-}

import Svg.Operations.Internal as Internal exposing (..)


{-| Spatially displace the foreground (second argument) with the color values of the background (third argument)
-}
displacement : Float -> FilterOperation msg -> FilterOperation msg -> FilterOperation msg
displacement scale =
...
-}
flood : { color : String, opacity : Float } -> FilterOperation msg
flood options =
...
{-| Apply a list of filters sequentially, only the effect of the last filter will be visible
-}
...
{-| Apply a list of filters sequentially, all filters' effects will be visible
-}
